...
public class CraftMenuScript : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        for (int i = 0;i<playerInventory.craftableItems.GetAllSlots().Length;i++)
        {
            ...
            temp.GetComponentInChildren<UnityEngine.UI.Button>().onClick.AddListener(()=> OnCraftItemSelected(temp.GetComponent<UICraftSlot>()));
    ...
    public void OnButtonClick(InventorySlot slot)
    {
    ...
    public void OnCraftItemSelected(UICraftSlot craftSlot)
    {
        ...
        InventorySlot slot = craftSlot.slot;
            ...
            (playerInventory.craftComponents.GetItemCount("Electronics") >= (slot.item.info as CraftableItemInfo).requeBolts))
        {
            ...
            bt.onClick.AddListener(() => OnButtonClick(slot));
        }
        else
        {
            //êðàñíóþ îáâîäêó äîáàâèòü â êíîïêó
            bt.interactable = false;
            bt.onClick.RemoveListener(() => OnButtonClick(slot));
        }    
